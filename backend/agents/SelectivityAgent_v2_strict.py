...
class SelectivityAgent:
    ...
    async def _execute(self, state: dict) -> dict:
        ...
        if not off_targets:
            log.warning("No off-target proteins available for selectivity assessment")
            return {}
    ...
    async def _prepare_receptor_pdbqt(self, pdb_content: str, pdb_id: str) -> str:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            output_path = f"/tmp/{pdb_id}_offtarget.pdbqt"
    ...
    async def _prepare_ligand_pdbqt(self, smiles: str) -> str:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            output_path = f"/tmp/ligand_{str(abs(hash(smiles)))[:8]}.pdbqt"
            shutil.copy(pdbqt_path, output_path)
