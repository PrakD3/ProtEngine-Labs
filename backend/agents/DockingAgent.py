    ...
    async def _execute(self, state: dict) -> dict:
        ...
        for idx, mol in enumerate(molecules[:30], start=1):
            ...
            energy, pose_meta = await _dock_one_async(
                smiles, pocket, receptor_pdbqt, exe, log, 
...
async def _dock_one_async(smiles, pocket, receptor_pdbqt, exe, log, **kwargs) -> tuple:
            ...
            with open(out_path) as f:
                for line in f:
                    if "REMARK VINA RESULT:" in line:
                        energy = float(line.split()[3]); break
...
def _vina_exe(mode):
    return shutil.which("gnina") if mode == "gnina" else (shutil.which("vina") or "vina")
