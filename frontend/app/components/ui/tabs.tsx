...
const Tabs = ({ defaultValue = "", value, onValueChange, children, className, ...props }: TabsProps) => {
  ...
  return <TabsContext.Provider value={{ value: current, onValueChange: onChange }}><div className={cn("", className)} {...props}>{children}</div></TabsContext.Provider>;
};

const TabsList = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("inline-flex h-10 items-center justify-start rounded-md bg-[var(--muted)] p-1 text-[var(--muted-foreground)] overflow-x-auto", className)} {...props} />
);
...
const TabsTrigger = ({ value, className, ...props }: TabsTriggerProps) => {
  ...
  return (
    <button
      ...
      className={cn("inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 shrink-0",
        ctx.value === value ? "bg-[var(--background)] text-[var(--foreground)] shadow-sm" : "hover:bg-[var(--background)]/50", className)}
      {...props}
    />
  );
};
