...
const DialogContent = ({ className, children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("bg-[var(--card)] rounded-lg shadow-xl border border-[var(--border)] p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto", className)} {...props}>{children}</div>
);
...
const DialogClose = ({ onClose }: { onClose: () => void }) => (
  <button type="button" onClick={onClose} className="absolute right-4 top-4 p-1 rounded hover:bg-[var(--muted)]"><X size={16} /></button>
);
